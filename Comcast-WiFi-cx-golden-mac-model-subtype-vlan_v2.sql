...
WITH basedata AS 
                ( SELECT  ap_mac_address,
                        ...
                        /* TCP RETRANSMISSION VALUES (TCP COUNT METHOD) */
                          SUM( CASE
                                    WHEN ( clientequipment_subscriber_type IS NOT NULL
                        ...
                        /* TCP RETRANSMISSION VALUES (ROW COUNT METHOD) */
                          SUM( CASE
                                    WHEN ( clientequipment_subscriber_type IS NOT NULL
                                        ...
                                        AND packet_type_name IN ( 'TCP' ) )
                                    THEN 1
                                    ELSE NULL END ) AS row_count_rtx_impacted,
                         /* TCP RETRANSMISSION VALUES (ROW COUNT METHOD) */
                          SUM( CASE
                                    WHEN ( upw_download_retransmitted_packets_count >= '0'
                                        ...
                                        AND upw_download_active_millis > '0' )
                                    THEN 1
                                    ELSE NULL END ) AS row_count_rtx,
                        /* THROUGHPUT VALUES */
                          SUM( CASE 
...
SELECT ap_mac_address AS 'ap_mac',
       ...
       ROUND(( 1 - ( basedata.row_count_rtx_impacted / NULLIFZERO(basedata.row_count_rtx) ) ) * 100, 2)  AS 'TCP-RTX % GOOD SESSIONS (2)',
       ...
       row_count_rtx AS 'total_rtx_rows',
       row_count_rtx_impacted AS 'total_rtx_rows_impacted'
FROM basedata
ORDER BY 1 DESC